...
class WechatApplet extends BaseObject
{
    ...
    public function getAccessToken(){
        ...
        if($access_token ){
            ...
            if($results){
                $cache->set("market_applet_access_token",$results["access_token"],$results["expires_in"]);
    ...
    public function getUserInfo($code){
        return json_decode($this->response($this->client->get("/sns/jscode2session",[
            'query'=>[
    ...
    public function response($response){
        ...
        if(!$response){
            throw new Exception('请求错误',$response['errcode']);
        }
