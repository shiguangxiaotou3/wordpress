...
#### 异步通知
如果收到商户的回复不符合规范或超时,会判定本次通知失败,重新发送通知,直到成功为止,但不保证通知最终一定能成功
异步通知用于:客户支付完成后,向下游服务器发送通知,下游服务器需回复 `success`或`error`或不回复
- 请求方式:post
- header:Content-type:application/x-www-form-urlencoded
...
#客户服务器在验证参数后,验证来源是否为上游服务器,并回复sueccss或error 或者不回复
~~~
