...
abstract class Generator extends Model
{
    ...
    {
        return [
            'template' => Yii::t('console','Template/view group'),
            'enableI18N' =>Yii::t('console', 'Enable I18N'),
            'messageCategory' =>Yii::t('console', 'Message Category'),
        ];
    }
    ...
    {
        return [
            'enableI18N' => Yii::t('console', 'This indicates whether the generator should generate strings using <code>Yii::t()</code> method. Set this to <code>true</code> if you are planning to make your application translatable.'),
            'messageCategory' => Yii::t('console', 'This is the category used by <code>Yii::t()</code> in case you enable I18N.'),
        ];
    }
    ...
    {
        return  Yii::t('console', 'The code has been generated successfully.');
    }
    ...
    {
        return [
            [['template'], 'required', 'message' =>Yii::t('console', 'A code template must be selected.')],
    ...
    public function validateTemplate()
    {
        ...
        if (!isset($templates[$this->template])) {
            $this->addError('template', Yii::t('console', 'Invalid template selection.'));
        } else {
...
//        }
    ...
    public function validateNewClass($attribute, $params)
    {
        ...
        if (($pos = strrpos($class, '\\')) === false) {
            ...
            if ($path === false) {
                $this->addError($attribute, Yii::t('console', "The class namespace is invalid: {ns}",['{ns}'=>$ns]));
            } elseif (!is_dir($path)) {
                $this->addError($attribute,  Yii::t('console',"Please make sure the directory containing this class exists: {path}",['{path}'=>$path]));
            }
        }
    }
    ...
    public function validateMessageCategory()
    {
        if ($this->enableI18N) {
            if (empty($this->messageCategory)) {
                $this->addError('messageCategory', Yii::t('console',"Message Category cannot be blank."));
            } elseif (!preg_match('~\w+~', $this->messageCategory)) {
                $this->addError('messageCategory', Yii::t('console',"Message Category is not valid. It should contain only alphanumeric characters and _."));
            }
        }
    }
