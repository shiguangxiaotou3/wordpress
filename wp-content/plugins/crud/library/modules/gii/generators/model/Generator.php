 ...
 * @since 2.0
 */
class Generator extends \crud\modules\gii\Generator
{
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            [['db', 'modelClass', 'queryClass'], 'match', 'pattern' => '/^\w+$/', 'message' => Yii::t('console','Only word characters are allowed.')],
            [['ns', 'baseClass', 'queryNs', 'queryBaseClass'], 'match', 'pattern' => '/^[\w\\\\]+$/', 'message' => Yii::t('console','Only word characters and backslashes are allowed.')],
            [['tableName'], 'match', 'pattern' => '/^([\w ]+\.)?([\w\* ]+)$/', 'message' => Yii::t('console','Only word characters, and optionally spaces, an asterisk and/or a dot are allowed.')],
            [['db'], 'validateDb'],
    ...
    public function hints()
    {
        return array_merge(parent::hints(), [
            'ns' => Yii::t('console','This is the namespace of the ActiveRecord class to be generated, e.g., <code>app\models</code>'),
            'db' =>Yii::t('console', 'This is the ID of the DB application component.'),
            'tableName' =>Yii::t('console', 'This is the name of the DB table that the new ActiveRecord class is associated with, e.g. <code>post</code>.The table name may consist of the DB schema part if needed, e.g. <code>public.post</code>.The table name may end with asterisk to match multiple table names, e.g. <code>tbl_*</code>will match tables who name starts with <code>tbl_</code>. In this case, multiple ActiveRecord classes will be generated, one for each matching table name; and the class names will be generated from the matching characters. For example, table <code>tbl_post</code> will generate <code>Post</code> class.'),
            'modelClass' =>Yii::t('console', 'This is the name of the ActiveRecord class to be generated. The class name should not contain the namespace part as it is specified in "Namespace". You do not need to specify the class name if "Table Name" ends with asterisk, in which case multiple ActiveRecord classes will be generated.'),
            'standardizeCapitals' =>Yii::t('console', 'This indicates whether the generated class names should have standardized capitals. For example, table names like <code>SOME_TABLE</code> or <code>Other_Table</code> will have class names <code>SomeTable</code> and <code>OtherTable</code>, respectively. If not checked, the same tables will have class names <code>SOMETABLE</code> and <code>OtherTable</code> instead.'),
            'singularize' =>Yii::t('console', 'This indicates whether the generated class names should have standardized capitals. For example,table names like <code>SOME_TABLE</code> or <code>Other_Table</code> will have class names <code>SomeTable</code>and <code>OtherTable</code>, respectively. If not checked, the same tables will have class names <code>SOMETABLE</code>and <code>OtherTable</code> instead.'),
            'baseClass' =>Yii::t('console', 'This is the base class of the new ActiveRecord class. It should be a fully qualified namespaced class name.'),
            'generateRelations' =>Yii::t('console', 'This indicates whether the generator should generate relations based on
                foreign key constraints it detects in the database. Note that if your database contains too many tables,
                you may want to uncheck this option to accelerate the code generation process.'),
            'generateJunctionRelationMode' =>Yii::t('console', 'This indicates whether junction relations are generated with `viaTable()` or `via()` (Via Model) relations.
                Make sure you also generate the junction models when using the "Via Model" option.'),
            'generateRelationsFromCurrentSchema' =>Yii::t('console', 'This indicates whether the generator should generate relations from current schema or from all available schemas.'),
            'useClassConstant' =>Yii::t('console', 'Use the `::class` constant instead of the `::className()` method.'),
            'generateLabelsFromComments' => Yii::t('console','This indicates whether the generator should generate attribute labels by using the comments of the corresponding DB columns.'),
            'useTablePrefix' =>Yii::t('console', 'This indicates whether the table name returned by the generated ActiveRecord class should consider the <code>tablePrefix</code> setting of the DB connection. For example, if the table name is <code>tbl_post</code> and <code>tablePrefix=tbl_</code>, the ActiveRecord class will return the table name as <code>{{%post}}</code>.'),
            'useSchemaName' => Yii::t('console','This indicates whether to include the schema name in the ActiveRecord class
                when it\'s auto generated. Only non default schema would be used.'),
            'generateQuery' => Yii::t('console','This indicates whether to generate ActiveQuery for the ActiveRecord class.'),
            'queryNs' =>Yii::t('console', 'This is the namespace of the ActiveQuery class to be generated, e.g., <code>app\models</code>'),
            'queryClass' =>Yii::t('console', 'This is the name of the ActiveQuery class to be generated. The class name should not contain
                the namespace part as it is specified in "ActiveQuery Namespace". You do not need to specify the class name
                if "Table Name" ends with asterisk, in which case multiple ActiveQuery classes will be generated.'),
            'queryBaseClass' => Yii::t('console','This is the base class of the new ActiveQuery class. It should be a fully qualified namespaced class name.'),
        ]);
    }
        ...
        if (empty($tables)) {
            ...
            foreach ($tables as $table) {
                ...
                if ($this->isReservedKeyword($class)) {
                    $this->addError('tableName', Yii::t('console',"Table '{table}' will generate a class which is a reserved PHP keyword.",['{table}'=>$table]));
                    break;
                }
            }
        }
